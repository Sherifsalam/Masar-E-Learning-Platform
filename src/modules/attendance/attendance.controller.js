...
const openSession = asyncHandler(async (req, res) => {
  const result = await service.openSession({ teacherId: req.user._id, subject: req.body.subject });
  sendResponse(res, 201, "Check-in session opened", result);
});

const checkIn = asyncHandler(async (req, res) => {
  const record = await service.checkIn({ studentId: req.user._id, sessionToken: req.body.sessionToken });
...
const myHistory = asyncHandler(async (req, res) => {
  ...
  const records = await service.getStudentHistory({ studentId: req.user._id, subject, from, to });
...
const classRoster = asyncHandler(async (req, res) => {
  ...
  const roster = await service.getClassRoster({ teacherId: req.user._id, subject, date });
  sendResponse(res, 200, "Class roster", roster);
});

const updateStatus = asyncHandler(async (req, res) => {
  const record = await service.updateStatus({ attendanceId: req.params.id, status: req.body.status });
